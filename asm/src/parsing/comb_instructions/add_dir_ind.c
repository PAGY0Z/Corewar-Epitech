...
////////////////////////////////////////////////////////////////////////////////
/// \file add_dir_ind.c
/// \brief Contains the function that add a direct or indirect argument to the
/// buffer.
...
bool add_dir_ind(asm_t *asm_struct, char *param, int cmd_pos, pbyte_t p_byte);

////////////////////////////////////////////////////////////////////////////////
/// \brief Add a direct or indirect argument to the buffer.
...
/// \note   - The argument is not a direct or an indirect.
/// \note   - The argument is a direct and an indirect.
////////////////////////////////////////////////////////////////////////////////
bool add_dir_ind(asm_t *asm_struct, char *param, int cmd_pos, pbyte_t p_byte)
{
    byte_t parsing_byte = asm_struct->buffer[cmd_pos + FULL_HEADER_SIZE + 1];
    bool is_direct = false; bool is_indirect = false;
    is_direct = add_direct(asm_struct, param, cmd_pos);
    is_indirect = add_indirect(asm_struct, param, cmd_pos);
    if ((is_direct + is_indirect) != 1) return false;
    if (p_byte) {
