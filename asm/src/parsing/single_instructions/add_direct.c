...
////////////////////////////////////////////////////////////////////////////////
/// \file add_direct.c
/// \brief Contains the function that adds a direct to the buffer.
...
////////////////////////////////////////////////////////////////////////////////
/// \brief Adds a direct to the buffer.
...
bool add_direct(asm_t *asm_struct, char *param, int cmd_position)
{
    ...
    if (str_is_number(param + 1))
        return add_four_bytes_to_buffer(asm_struct, my_getnbr(param + 1));
    if (is_registered_label2(asm_struct, param + 1)) {
        if (!(call = init_call_data(cmd_position, asm_struct->index, 4)))
            return false;
        ...
        my_free(call);
        return add_four_bytes_to_buffer(asm_struct, 0);
    }
    return false;
}
