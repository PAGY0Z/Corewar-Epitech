...
bool is_registered_label(asm_t *asm_struct, char *label_name)
{
    ...
    if ((str_len = my_strlen(label_name)) < 2) return false;
    if (label_name[str_len - 1] != ':') return false;
    label_name[str_len - 1] = '\0';
    for (int i = 0; asm_struct->labels[i]; i++) {
        if (my_strcmp(asm_struct->labels[i]->name, label_name)) {
            label_name[str_len - 1] = ':';
            return true;
        }
    }
    label_name[str_len - 1] = ':';
